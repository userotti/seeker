...
class LevelBuilder {
  ...
  //STATIC TOWERS ===================================================
  public function makeMetalNest (_static_tower:TextureSprite, _pos:Vector){
  ...
  //TOWERS =================================================
  public function makeMinorRedHexGrunt(_tower:TextureSprite, _pos:Vector){

  }

  public function makeMinorGreenRectGrunt(_tower:TextureSprite, _pos:Vector){
    _tower.pos = _pos;
  ...
  public function makeMinorBlueTriGrunt(_tower:TextureSprite, _pos:Vector){
    _tower.pos = _pos;
    ...
    _tower.get(BoostComponent.TAG).setup(310, 140000, 1500); //boostpower, top_speed, max_fuel, fuel_recharge
    _tower.get(ForceBodyComponent.TAG).setup(1);//hit_radius
    _tower.get(ForceFieldComponent.TAG).setup(100, 50, 150);//bigradius, small_radius constant_force
    _tower.get(DefenceComponent.TAG).setup(2, 100);//bigradius, small_radius
    _tower.get(OffenceComponent.TAG).setup(200, 50, 5, 200);//bigradius, small_radius, , ddudeamage, reload
  }

  //PUSAHBLES ===========================================
  public function makeOre(_tower:TextureSprite, _pos:Vector){
    _tower.pos = _pos;
  ...
  public function makeOrePatch(_amount:Int, _w:Int, _h:Int, _pos:Vector, csm:CollisionSceneManager){
    for(i in 0..._amount){
